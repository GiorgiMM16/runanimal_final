...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Alert, ActivityIndicator, Platform, Modal } from 'react-native';
import { MapPin, Navigation, Locate, Flame, Square, Heart, Timer, Footprints, Share2, X } from 'lucide-react-native';
...
export default function MapScreen() {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => {
            const collectedCharacters = characters.filter(c => c.collected);
            setSummaryData({
              activityType: activityState.activityType,
              distance: activityState.distance,
              speed: activityState.speed,
              elapsedTime: activityState.elapsedTime,
              path: activityState.path,
              collectedCharacters,
            });
            setShowSummary(true);
            endActivity();
  ...
  const handleShare = async () => {
    try {
      ...
      const uri = await captureRef(summaryViewRef, {
        format: 'png',
        quality: 1,
      });
      ...
      if (Platform.OS === 'web') {
        ...
        link.download = 'activity-summary.png';
        link.href = uri;
        link.click();
      } else {
  ...
  const generateSummaryMapHTML = () => {
    if (!summaryData || !summaryData.path || summaryData.path.length === 0) return '';

    const allLats = summaryData.path.map((p: [number, number]) => p[0]);
    const allLngs = summaryData.path.map((p: [number, number]) => p[1]);
    const centerLat = (Math.min(...allLats) + Math.max(...allLats)) / 2;
    const centerLng = (Math.min(...allLngs) + Math.max(...allLngs)) / 2;

    const pathCoordinates = summaryData.path.map(([lat, lng]: [number, number]) => `[${lat}, ${lng}]`).join(',');
    ...
    return `
      ...
      <body>
        ...
        <script>
          ...
          var pathCoords = [${pathCoordinates}];
          if (pathCoords.length > 1) {
            L.polyline(pathCoords, { color: '#FF6B35', weight: 4, opacity: 0.8 }).addTo(map);
            map.fitBounds(pathCoords, { padding: [20, 20] });
          }
          if (pathCoords.length > 0) {
            L.circleMarker(pathCoords[0], { radius: 6, fillColor: '#4ADE80', color: '#FFFFFF', weight: 2, opacity: 1, fillOpacity: 1 }).addTo(map);
            L.circleMarker(pathCoords[pathCoords.length - 1], { radius: 6, fillColor: '#DC2626', color: '#FFFFFF', weight: 2, opacity: 1, fillOpacity: 1 }).addTo(map);
          }
        </script>
      </body>
      </html>
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal visible={showSummary} animationType="slide" transparent={false}>
        <View style={styles.summaryContainer} ref={summaryViewRef} collapsable={false}>
          ...
          <View style={styles.summaryContent}>
            <View style={styles.activityTypeSection}>
              <Text style={styles.activityTypeLabel}>{summaryData?.activityType?.toUpperCase()}</Text>
            </View>
            ...
            <View style={styles.summaryMapSection}>
              {Platform.OS === 'web' ? (
                <iframe
                  ...
                  srcDoc={generateSummaryMapHTML()}
                />
              ) : (
                <WebView
                  style={styles.summaryMap}
                  source={{ html: generateSummaryMapHTML() }}
                  scrollEnabled={false}
                />
              )}
            </View>
            ...
            <View style={styles.summaryStatsGrid}>
              <View style={styles.summaryStatCard}>
                <Timer size={24} color="#FF6B35" />
                <Text style={styles.summaryStatValue}>{formatTime(summaryData?.elapsedTime || 0)}</Text>
                <Text style={styles.summaryStatLabel}>Time</Text>
              </View>
              <View style={styles.summaryStatCard}>
                <Footprints size={24} color="#4A90E2" />
                <Text style={styles.summaryStatValue}>{(summaryData?.distance || 0).toFixed(2)} km</Text>
                <Text style={styles.summaryStatLabel}>Distance</Text>
              </View>
              <View style={styles.summaryStatCard}>
                <Navigation size={24} color="#8B7355" />
                <Text style={styles.summaryStatValue}>{(summaryData?.speed || 0).toFixed(1)} km/h</Text>
                <Text style={styles.summaryStatLabel}>Avg Speed</Text>
              </View>
            </View>

            <View style={styles.charactersSection}>
              <Text style={styles.charactersSectionTitle}>Runimals Collected</Text>
              <View style={styles.charactersGrid}>
                {summaryData?.collectedCharacters?.map((char: CharacterMarker) => (
                  <View key={char.id} style={styles.characterCard}>
                    <Text style={styles.characterEmoji}>{getElementIcon(char.type)}</Text>
                    <Text style={styles.characterName}>{char.name}</Text>
                  </View>
                ))}
                {(!summaryData?.collectedCharacters || summaryData.collectedCharacters.length === 0) && (
                  <Text style={styles.noCharactersText}>No runimals collected</Text>
                )}
              </View>
              <Text style={styles.charactersCount}>
                Total: {summaryData?.collectedCharacters?.length || 0}
              </Text>
            </View>
          </View>
